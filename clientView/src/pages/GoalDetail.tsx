...
// ─── Dummy recipe fallback (until AI-generated values are stored in DB) ────────

const DUMMY_RECIPE = {
  recipeVideoId: "bJUr9c0v3s8",
  recipeUrl: "https://www.youtube.com/watch?v=bJUr9c0v3s8",
  recipeThumbnail: "https://i.ytimg.com/vi/bJUr9c0v3s8/hq720.jpg",
  recipeTitle: "Healthy Meal Prep Recipe",
  recipeChannelName: "Cooking Channel",
};
...
const MealCard = ({
  ...
  // Resolve recipe fields with fallback (mirrors ExerciseCard's DUMMY_TUTORIAL pattern)
  const recipe = {
    recipeUrl:         task.recipeUrl?.trim()    || (task.recipeVideoId ? `https://www.youtube.com/watch?v=${task.recipeVideoId}` : "") || DUMMY_RECIPE.recipeUrl,
    recipeThumbnail:   task.recipeThumbnail      || DUMMY_RECIPE.recipeThumbnail,
    recipeTitle:       task.recipeTitle           || `${task.title} Recipe`,
    recipeChannelName: task.recipeChannelName     || DUMMY_RECIPE.recipeChannelName,
  };
  ...
  useEffect(() => {
    const fetchGoal = async () => {
      try {
        ...
        console.log("[GoalDetail] Fetched goal recipe fields:", res.data?.mealPlan?.map((day: any) =>
          day.meals.map((meal: any) => ({
            title: meal.title,
            recipeUrl: meal.recipeUrl,
            recipeVideoId: meal.recipeVideoId,
            recipeThumbnail: meal.recipeThumbnail,
            recipeTitle: meal.recipeTitle,
            recipeChannelName: meal.recipeChannelName,
          }))
        ));
        setGoal(res.data);
      } catch (err: any) {
