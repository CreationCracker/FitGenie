                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        {(meal.recipeUrl || meal.recipeVideoId) && (
                          <a
                            href={meal.recipeUrl || `https://www.youtube.com/watch?v=${meal.recipeVideoId}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-1 text-[10px] font-semibold bg-red-500/10 text-red-600 px-2 py-0.5 rounded-lg hover:bg-red-500/20 transition-colors"
                          >
                            <Youtube className="w-3 h-3" /> Recipe
                          </a>
                        )}
                      </div>
                    </div>
...
const PlanFeedback = () => {
  ...
  const handleApprove = useCallback(async () => {
    ...
    try {
      ...
      // Debug: confirm the meal recipe fields before saving
      console.log("[PlanFeedback] Saving meal recipe fields:", plan?.mealPlan?.map((day) =>
        day.meals.map((meal) => ({
          title: meal.title,
